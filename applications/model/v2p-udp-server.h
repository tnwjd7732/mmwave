...
#ifndef V2P_UDP_SERVER_H
#define V2P_UDP_SERVER_H
...
extern std::map <int32_t, std::vector<double>> mecDbPed;
extern std::map <int32_t, std::vector<double>> mecDbVeh;
class V2PUdpServer : public Application
{
public:
  /**
...
private:
  ...
  void CollisionPredict(void);
/**
   * \performing collision prediction algorithm with the received data
   * \param -> pedestrian's position and velocity data
   */
  double CalculateAnglePoint(double x1, double x2, double cx, double y1, double y2, double cy);
  double cot(double i);
  void CollisionPredictionAlgorithm (uint32_t pedNodeID, double pos_x, double pos_y, double velo_x, double velo_y, double angle);

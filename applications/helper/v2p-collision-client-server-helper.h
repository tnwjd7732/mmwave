 ...
 */
#ifndef V2P_COLLISION_CLIENT_SERVER_HELPER_H
#define V2P_COLLISION_CLIENT_SERVER_HELPER_H
 ...
 *
 */
class V2PCollisionClientHelper
{
...
public:
  ...
  V2PCollisionClientHelper (Address ip, uint16_t port);
  /**
   *  Create UdpClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
  ...
  V2PCollisionClientHelper (Address addr);
...
class V2PCollisionServerHelper
{
public:
  /**
   ...
   */
  V2PCollisionServerHelper (uint16_t port);
   ...
   */
  Ptr<V2PCollisionServer> GetServer (void);
private:
  ObjectFactory m_factory; //!< Object factory.
  Ptr<V2PCollisionServer> m_server; //!< The last created server application
};
...
*/
class V2PCollisionTraceClientHelper
{
public:
  /**
   ...
   */
  V2PCollisionTraceClientHelper (Address ip, uint16_t port, std::string filename);
  /**
   * Create UdpTraceClientHelper which will make life easier for people trying
   * to set up simulations with udp-client-server. Use this variant with
   * addresses that do include a port value (e.g., InetSocketAddress and
   ...
   * \param filename the file from which packet traces will be loaded
   */
  V2PCollisionTraceClientHelper (Address addr, std::string filename);
